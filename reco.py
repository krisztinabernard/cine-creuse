...
if submit_form:
    ...
    if resultat is not None:
        st.subheader(f"Top 10 des films similaires à **{titre.title()}** :")
        ...
        for index, row in resultat.iterrows():
            ...
            with st.expander(header_title):
                ...
                # Crée deux colonnes : 1 pour l'image (25%), 3 pour les détails (75%)
                ...
                with info_col:
                    ...
                    st.markdown("---") # Ligne de séparation courte pour les métriques
                    
                    # Métriques regroupées
                    ...
                    with colB:
                        if pd.notna(row['langue_originale']):
                            st.markdown(f"**Langue Originale :** {row['langue_originale']}")
